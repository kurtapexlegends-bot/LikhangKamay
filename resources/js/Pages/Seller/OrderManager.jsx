...
const sellerCourierTrackingState = (order) => {
    ...
    if (String(order?.delivery?.status || '').toUpperCase() === 'COMPLETED' && order?.status === 'Delivered') {
        return {
            ...base,
            label: isReplacementExchange ? 'Exchange Completed' : 'Awaiting Buyer',
            tone: 'border-emerald-200 bg-emerald-50 text-emerald-700',
    ...
    if (String(order?.delivery?.status || '').toUpperCase() === 'COMPLETED' && order?.status === 'Completed') {
        return {
            ...base,
            label: isReplacementExchange ? 'Exchange Resolved' : 'Buyer Confirmed',
...
export default function OrderManager({ auth, orders = [] }) {
    ...
    return (
        <>
                                                ...
                                                <div className="space-y-2">
                                                        ...
                                                        <div className="space-y-2">
                                                            <div className="flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 px-2.5 py-2">
                                                                ...
                                                                <div className="min-w-0">
                                                                    ...
                                                                    <p className="text-[9px] text-green-600">Complete when buyer has confirmed.</p>
                                                                </div>
                                                            </div>
                                                            <button
                                                                disabled={!canEditOrders}
                                                    ...
                                                    {order.status === 'Delivered' && !order.replacement_in_progress && isLalamoveManagedOrder(order) && (
                                                        <div className="flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-2">
                                                            ...
                                                            <div className="min-w-0">
                                                                ...
                                                                <p className="text-[9px] text-emerald-600">Awaiting buyer receipt confirmation.</p>
                                                            </div>
                                                        </div>
                                                    )}
                                                    ...
                                                    {order.status === 'Delivered' && order.replacement_in_progress && (
                                                        <div className="flex items-center gap-2 rounded-lg border border-teal-200 bg-teal-50 px-2.5 py-2">
                                                            ...
                                                            <div className="min-w-0">
                                                                ...
                                                                <p className="text-[9px] text-teal-600">Replacement unresolved until receipt.</p>
                                                            </div>
                                                        </div>
                                                    )}
                                                    ...
                                                    {(order.status === 'Cancelled' || order.status === 'Rejected') && (
                                                        <div className="flex items-center justify-center gap-1.5 rounded-lg bg-gray-50 border border-gray-100 px-2.5 py-2">
                                                            ...
                                                            <p className="text-[11px] font-medium text-gray-500">Order {order.status.toLowerCase()}</p>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
            ...
            {(() => {
                const orderToShip = orders.find(o => o.id === shippingModal.orderId);
                return (
                    <Modal show={shippingModal.isOpen} onClose={closeShippingModal} maxWidth="5xl">
                            ...
                            <div className="shrink-0 border-b border-stone-200/60 bg-white px-6 py-5 flex items-center justify-between sticky top-0 z-20">
                                <div className="flex items-center gap-4">
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                                        ...
                                        {shippingModal.status === 'Delivered' ? <MapPin size={24} /> : shippingModal.isPickup ? <PackageCheck size={24} /> : <Truck size={24} />}
                                    </div>
                                    <div>
                                        ...
                                        <div className="mt-0.5 flex items-center gap-2">
                                            <span className="text-[10px] font-bold uppercase tracking-widest text-stone-400">Order</span>
                                            <span className="rounded bg-stone-100 px-2 py-0.5 font-mono text-[11px] font-bold text-stone-600">{shippingModal.orderId}</span>
                                        </div>
                                    </div>
                                </div>
                                <button type="button" onClick={closeShippingModal} className="p-2 text-stone-400 hover:bg-stone-100 hover:text-stone-900 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-stone-200">
                                    <X size={20} />
                                </button>
                            </div>
                                            ...
                                            <>
                                                ...
                                                <section className="bg-white rounded-2xl border border-stone-200 p-5 shadow-sm">
                                                    <h3 className="text-sm font-bold text-stone-900 uppercase tracking-wider border-b border-stone-100 pb-2 mb-4 flex items-center gap-2">
                                                        <User size={16} className="text-stone-400" /> Destination
                                                    </h3>
                                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                        <div>
                                                            <p className="text-[10px] font-bold uppercase tracking-wider text-stone-400 mb-1">Buyer Name</p>
                                                            <p className="text-sm font-bold text-stone-900">{orderToShip.customer}</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-[10px] font-bold uppercase tracking-wider text-stone-400 mb-1">Contact</p>
                                                            <p className="text-sm font-medium text-stone-700">{orderToShip.shipping_contact_phone || 'No phone provided'}</p>
                                                        </div>
                                                        <div className="sm:col-span-2 mt-2">
                                                            <div className="flex items-start gap-2.5 rounded-xl bg-stone-50 p-3 border border-stone-100">
                                                                ...
                                                                <p className="text-sm font-medium text-stone-800 leading-relaxed">{orderToShip.shipping_address || 'No address provided (Pickup)'}</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </section>
                                                ...
                                                {orderToShip.buyer_notes && (
                                                    <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 shadow-sm">
                                                        <div className="flex items-center gap-2 mb-2">
                                                            ...
                                                            <h3 className="text-sm font-bold text-amber-900 uppercase tracking-wider">Buyer Notes</h3>
                                                        </div>
                                                        <p className="text-sm text-amber-800 italic">"{orderToShip.buyer_notes}"</p>
                                                    </div>
                                                )}
                                                ...
                                                <section>
                                                    <h3 className="text-sm font-bold text-stone-900 uppercase tracking-wider border-b border-stone-200 pb-2 mb-4 flex items-center gap-2">
                                                        <PackageOpen size={16} className="text-stone-400" /> Items to Pack
                                                    </h3>
                                                    <div className="space-y-3">
                                                        {orderToShip.items.map((item, idx) => (
                                                            <div key={idx} className="flex items-center gap-4 rounded-xl border border-stone-100 bg-white p-3 shadow-sm hover:border-stone-200 transition-colors">
                                                                ...
                                                                <div className="flex-1 min-w-0">
                                                                    ...
                                                                    <p className="text-xs text-stone-500 font-medium">Variant: {item.variant}</p>
                                                                </div>
                                                                <div className="shrink-0 text-right">
                                                                    <p className="text-[10px] font-bold uppercase tracking-wider text-stone-400 mb-0.5">Quantity</p>
                                        ...
                                        <div className="bg-white border border-stone-200 rounded-2xl p-5 sm:p-6 shadow-sm flex-1">
                                            ...
                                            <div className={`mb-6 rounded-xl border px-4 py-3 text-xs leading-relaxed ${
                                                ...
                                                <p className="font-bold uppercase tracking-[0.16em] mb-1">
                                                    {shippingModal.status === 'Delivered' ? 'Next step' : shippingModal.isPickup ? 'Pickup flow' : 'Shipping flow'}
                                                </p>
                                            ...
                                            {shippingModal.allowTracking && (
                                                <div className="mb-6">
                                                    <label className="block text-sm font-bold text-stone-700 mb-2">
                                                        <Hash size={14} className="inline mr-1 text-stone-400" /> Tracking Number
                                                    </label>
                                                    <TextInput 
                                                        disabled={!canEditOrders}
                                            ...
                                            <div className="mb-6">
                                                <label className="flex items-center justify-between text-sm font-bold text-stone-700 mb-2">
                                                    ...
                                                    {shippingModal.proofRequired && <span className="text-[10px] uppercase tracking-wider text-amber-600 bg-amber-50 px-2 py-0.5 rounded border border-amber-200">Required</span>}
                                                </label>
                                                <div className="relative w-full border-2 border-dashed border-stone-300 rounded-xl p-4 text-center cursor-pointer hover:border-stone-400 hover:bg-stone-50 transition group overflow-hidden bg-stone-50/30">
                                                    ...
                                                    {shippingModal.previewUrl ? (
                                                        <div className="relative">
                                                            <img src={shippingModal.previewUrl} alt="Proof" className="h-40 w-full object-cover mx-auto rounded-lg shadow-sm" />
                                                            <div className="absolute inset-0 bg-stone-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px] rounded-lg">
                                                                <span className="bg-white text-stone-900 px-4 py-2 rounded-xl text-xs font-bold shadow-lg">Change Photo</span>
                                                            </div>
                                                        </div>
                                                    ) : (
                                                        <div className="py-6 flex flex-col items-center justify-center text-stone-400 group-hover:text-stone-600 transition-colors">
                                                            ...
                                                            <p className="text-sm font-bold text-stone-700">Click to upload photo</p>
                                                            <p className="text-[10px] font-medium mt-1 max-w-[200px] text-center leading-snug">{shippingModal.proofHint}</p>
                                                        </div>
                                                    )}
                                                </div>
                                                {shippingModal.existingProofUrl && (
                                                    <div className="mt-3 flex items-center gap-2 rounded-lg bg-stone-50 p-2.5 border border-stone-100">
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-[11px] font-bold text-stone-700 truncate">Existing proof attached</p>
                                                            {shippingModal.proofRequired && <p className="text-[9px] text-stone-500">Upload a new one to replace it.</p>}
                                                        </div>
                                                        <a href={shippingModal.existingProofUrl} target="_blank" rel="noopener noreferrer" className="shrink-0 text-[10px] font-bold text-clay-600 hover:underline">View</a>
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <div>
                                                <label className="block text-sm font-bold text-stone-700 mb-2">
                                                    {shippingModal.noteLabel} <span className="text-stone-400 font-normal text-xs">(Optional)</span>
                                                </label>
                                                <textarea 
                                                    disabled={!canEditOrders}

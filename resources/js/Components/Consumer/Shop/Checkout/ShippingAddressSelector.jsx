...
export default function ShippingAddressSelector({
    ...
    const [showAddressSelectorMobile, setShowAddressSelectorMobile] = useState(false);

    const selectedAddress = auth.user.addresses?.find((addr) => addr.id === data.selected_address_id) || null;
        ...
        <div className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm sm:p-5">
            <div className="mb-3 flex items-center justify-between">
                ...
                {!!auth.user.addresses?.length && (
                    <button
                        type="button"
                        onClick={() => setShowAddressSelectorMobile(true)}
                        className="text-xs font-bold text-clay-600 hover:text-clay-750 transition md:hidden"
                    >
                        Change Address
                    </button>
                )}
            ...
            {/* Saved Addresses (Desktop: Grid, Mobile: Selected Active Card) */}
            {!!auth.user.addresses?.length ? (
                <>
                    {/* Desktop View: Full Grid */}
                    <div className="hidden md:grid md:grid-cols-2 md:gap-4 mb-4">
                    ...
                    {/* Mobile View: Active Selected Card */}
                    <div className="md:hidden mb-4">
                        {selectedAddress ? (
                            <div
                                onClick={() => setShowAddressSelectorMobile(true)}
                                className="cursor-pointer rounded-xl border border-clay-600 bg-clay-50/10 ring-1 ring-clay-600 p-4 transition-all hover:bg-clay-50/25 shadow-sm"
                            >
                                <div className="flex items-start gap-3">
                                    <div className="shrink-0 mt-1">
                                        <div className="h-4 w-4 rounded-full border border-clay-600 bg-clay-600 flex items-center justify-center">
                                            <div className="h-1.5 w-1.5 rounded-full bg-white" />
                                        </div>
                                    </div>
                                    <div className="flex-1 space-y-1.5 min-w-0">
                                        <div className="flex items-center gap-2">
                                            <span className="text-sm font-bold text-gray-800 truncate">{selectedAddress.label}</span>
                                            <span className="shrink-0 rounded-full border border-blue-100 bg-blue-50 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-blue-700">
                                                {typeLabel(selectedAddress.address_type)}
                                            </span>
                                        ...
                                        <p className="line-clamp-2 text-xs text-gray-650 leading-relaxed">{resolveAddressDisplay(selectedAddress)}</p>
                                        <p className="text-[10px] font-semibold text-gray-500 tracking-wide">
                            ...
                            <div 
                                ...
                                className="flex cursor-pointer items-center justify-between rounded-xl border border-dashed border-clay-600 bg-clay-50/20 p-4 text-clay-700 shadow-sm"
                            >
                                <span className="text-sm font-bold text-gray-805">New Address Details Required</span>
                                <span className="text-xs font-bold text-clay-600">Enter Details</span>
                            </div>
                        )}
                    </div>
                </>
            ...
            {/* Mobile Address Selection Drawer */}
            <SlideOverDrawer
                show={showAddressSelectorMobile}
                onClose={() => setShowAddressSelectorMobile(false)}
            ...
            >
                <div className="space-y-4 max-h-[70vh] overflow-y-auto pb-6">
                    {auth.user.addresses?.map((address) => (
                        <div
                            ...
                            onClick={() => {
                                ...
                                setShowAddressSelectorMobile(false);
                            }}
                        ...
                        >
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 space-y-2 min-w-0">
                                    <div className="flex items-center justify-between gap-2">
                                        <div className="flex items-center gap-2 min-w-0">
                                            <span className="text-sm font-bold text-gray-805 truncate">{address.label}</span>
                                            <span className="shrink-0 rounded-full border border-blue-100 bg-blue-50 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-blue-700">
                                                {typeLabel(address.address_type)}
                                            </span>
                                        </div>
                                    ...
                                    <p className="text-xs text-gray-655 leading-relaxed">{resolveAddressDisplay(address)}</p>
                                    <p className="text-[10px] font-semibold text-gray-500 tracking-wide">
                                        {address.recipient_name} • {address.phone_number}
                                    </p>
                                </div>
                            ...
                            {/* Separated Actions Footer */}
                            <div className="mt-3 pt-3 border-t border-stone-100 flex items-center justify-between gap-2" onClick={(event) => event.stopPropagation()}>
                                <div className="flex items-center gap-1">
                                    {!address.is_default && (
                                        <button
                                            type="button"
                                            onClick={() => setDefaultAddress(address.id)}
                                            className="h-8 px-2 rounded-lg text-xs font-bold text-gray-500 hover:bg-stone-50 hover:text-clay-600 transition"
                                        >
                                            Set Default
                                        </button>
                                    )}
                                ...
                                <div className="flex items-center gap-1">
                                    <button
                                        ...
                                        className="inline-flex h-8 px-2 items-center gap-1 rounded-lg text-xs font-bold text-gray-500 hover:bg-stone-50 hover:text-clay-600 transition"
                                    >
                                        <Pencil size={11} />
                                        Edit
                                    </button>
                                    <button
                                        type="button"
                                        onClick={() => setDeleteAddressTarget(address)}
                                        className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-stone-50 hover:text-red-650 transition"
                                        aria-label={`Delete ${address.label} address`}
                                        title="Delete address"
                                    >
                                        <Trash2 size={13} />
                    ...
                    <div 
                        ...
                        className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border-2 border-dashed border-gray-200 p-4 text-center text-gray-400 hover:border-clay-400 hover:text-clay-600 transition"
                    >

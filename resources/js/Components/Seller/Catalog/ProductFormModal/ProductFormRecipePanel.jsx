...
import { Settings, Plus, Trash2, Store, Crown, Sparkles } from "lucide-react";
...
export default function ProductFormRecipePanel({
    ...
    const { sellerSubscription, auth } = usePage().props;
    const currentTierKey = sellerSubscription?.tierKey 
        || auth?.user?.premium_tier 
        || auth?.user?.subscription_tier 
        || 'free';

    const hasProcurementAccess = currentTierKey === 'premium' || currentTierKey === 'super_premium';
            ...
            <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
                ...
                {data.production_method === "manufactured" ? (
                    hasProcurementAccess ? (
                        ...
                        <div className="rounded-2xl border border-stone-200 bg-stone-50/60 p-6 text-center space-y-4">
                            <div className="mx-auto w-12 h-12 rounded-2xl bg-amber-50 border border-amber-200/80 flex items-center justify-center text-amber-700 shadow-2xs">
                                <Crown size={22} />
                            </div>

...
export default function ReleaseRequestModal({
            ...
            <div className="min-w-0">
                ...
                <p className="mt-1 text-[11px] font-medium text-stone-500 sm:text-[12px]">
                    {isSale
                        ...
                        : 'Review the stored breakdown and any rejection reason.'}
                </p>
            </div>
        </div>
    );

    // Right Action Header/Summary widget (useful for desktop view layout)
    const rightHeaderAction = (
        <div className="flex items-center gap-3 self-center ml-auto pr-2">
            {isPayroll && (
    ...
    // Modal/Drawer Footer
    const modalFooter = isPendingReview ? (
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex-1 w-full sm:max-w-md">
                <input
                    type="text"
                    disabled={!canEditAccounting || !!reviewProcessing}
                    className="w-full rounded-xl border-stone-200 py-2.5 px-4 text-[12px] font-medium transition focus:border-red-400 focus:ring-red-400 disabled:cursor-not-allowed disabled:bg-stone-50 disabled:text-stone-400 shadow-sm min-h-[44px]"
            ...
            <div className="flex items-center justify-end gap-2 shrink-0 w-full sm:w-auto">
                <button
                    ...
                    onClick={onClose}
                    className="rounded-xl border border-stone-200 bg-white px-4 py-2.5 text-[11px] font-bold uppercase tracking-widest text-stone-500 transition hover:bg-stone-50 hover:text-stone-700 min-h-[44px] disabled:cursor-not-allowed disabled:opacity-50"
                >
                    Cancel
                </button>
                <button
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-end w-full gap-3">
            {isSale && (
                <button
                    type="button"
            ...
            <button
                type="button"
                onClick={onClose}
                className="rounded-xl bg-stone-900 border border-transparent px-5 py-2.5 text-[11px] font-bold uppercase tracking-widest text-white shadow-sm transition hover:bg-clay-700 min-h-[44px] w-full sm:w-auto"
            >
                Close Verification
            </button>
        </div>
    ...
    return (
        <SlideOverDrawer
            show={isOpen}
            onClose={onClose}
            widthClass={isPayroll ? 'max-w-5xl' : isSale ? 'max-w-3xl' : 'max-w-2xl'}
                                    ...
                                    { label: 'Total Cost', value: formatMoney(item.amount) },
                                    {
                                        ...
                                        value: (
                                            <div className="text-right">
                                                ...
                                                <div className={`text-[10px] font-bold uppercase tracking-wider mt-0.5 ${Number(item.fund_snapshot?.remaining_balance) < 0 ? 'text-red-650' : 'text-emerald-650'}`}>
                                    ...
                                    <table className="w-full min-w-[720px] text-left border-collapse">
                                        ...
                                        <tbody className="divide-y divide-stone-100">
                                            {item.line_items.map((line) => (
                                                ...
                                                >
                                                    <td className="px-6 py-4 font-bold text-stone-900 sticky left-0 bg-white group-hover:bg-stone-50 transition-colors z-10 border-r border-stone-100 shadow-[2px_0_5px_rgba(0,0,0,0.02)]">
                                                        {line.employee_name}
                                                    </td>
                                                    <td className="px-4 py-4 text-right">{formatMoney(line.base_salary)}</td>
                                                    <td className="px-4 py-4 text-right">{line.days_worked}</td>
                                                    <td className="px-4 py-4 text-right text-rose-600 font-semibold">
                                                    ...
                                                    <td className="px-6 py-4 text-right text-[14px] font-bold tracking-tight text-emerald-800 bg-emerald-50/30">

...
function ActionControl({ label, href, onClick, tone = 'primary' }) {
    ...
    const baseClassName = tone === 'secondary'
        ? 'inline-flex items-center justify-center rounded-xl border border-stone-200 bg-white px-4 py-2 text-[12px] font-bold text-stone-700 shadow-sm transition hover:bg-stone-50'
        : 'inline-flex items-center justify-center rounded-xl bg-clay-600 px-4 py-2 text-[12px] font-bold text-white shadow-sm transition hover:bg-clay-700';
...
export default function WorkspaceEmptyState({
    ...
    return (
        <div className={`flex flex-col items-center justify-center text-center ${compact ? 'px-6 py-10' : 'px-8 py-14'}`}>
            <div className={`inline-flex items-center justify-center rounded-2xl border shadow-sm ${compact ? 'h-12 w-12' : 'h-16 w-16'} border-stone-200 bg-[#FCF7F2] text-clay-500`}>

...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; margin: 0; padding: 0; background-color: #FDFBF9; color: #2E2520; line-height: 1.6; -webkit-text-size-adjust: none;">
    <div style="max-width: 600px; margin: 40px auto; background-color: #ffffff; border: 1px solid #E7E1D8; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 12px rgba(46, 37, 32, 0.03);">
        ...
        <div style="padding: 40px 32px;">
            <h1 style="font-family: Georgia, Times, serif; font-size: 24px; font-weight: normal; color: #A2582F; margin-top: 0; margin-bottom: 8px; text-align: center;">New Order Received!</h1>
            <div style="text-align: center;">
                <span style="display: inline-block; background-color: #FDFBF9; border: 1px solid #E7E1D8; color: #A2582F; padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: 600; margin: 12px 0 24px;">Awaiting Action</span>
            </div>

            <p style="margin-top: 0; margin-bottom: 16px; font-size: 15px; color: #5C524A; line-height: 1.6;">Hello Artisan,</p>
            <p style="margin-top: 0; margin-bottom: 16px; font-size: 15px; color: #5C524A; line-height: 1.6;">Great news! You have received a new order from <strong>{{ $order->customer_name }}</strong>.</p>
            ...
            <div style="background-color: #FFFDFB; border: 1px solid #E7E1D8; border-radius: 8px; padding: 24px; margin: 24px 0;">
                ...
                <p style="color: #8C827A; font-size: 13px; margin-top: -8px; margin-bottom: 16px;">
                    Placed: {{ $order->created_at->format('F d, Y • h:i A') }}
                </p>
            ...
            <div style="margin-top: 24px; border-top: 1px solid #E7E1D8; padding-top: 20px;">
                <div style="font-family: Georgia, Times, serif; font-size: 15px; font-weight: bold; color: #2E2520; margin-bottom: 8px;">Shipping Address</div>
                <p style="font-size: 14px; color: #5C524A; margin-bottom: 16px; line-height: 1.5;">{{ $order->shipping_address }}</p>

                @if($order->shipping_notes)
                <div style="font-family: Georgia, Times, serif; font-size: 15px; font-weight: bold; color: #2E2520; margin-bottom: 8px;">Customer Notes</div>
                <p style="font-size: 14px; color: #5C524A; margin-bottom: 16px; line-height: 1.5;">{{ $order->shipping_notes }}</p>
                @endif

                <div style="font-family: Georgia, Times, serif; font-size: 15px; font-weight: bold; color: #2E2520; margin-bottom: 8px;">Payment Method</div>
                <p style="font-size: 14px; color: #5C524A; margin-bottom: 0;">{{ $order->payment_method }}</p>
            </div>

            <p style="margin-top: 28px; text-align: center; font-size: 15px; color: #5C524A; line-height: 1.6;">Please review and accept the order, then communicate with the customer via chat to coordinate shipping.</p>

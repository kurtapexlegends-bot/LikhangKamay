...
<head>
    <meta charset="utf-8">
    <title>Product Moderation Result</title>
</head>
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; margin: 0; padding: 0; background-color: #FDFBF9; color: #2E2520; line-height: 1.6; -webkit-text-size-adjust: none;">
    <div style="max-width: 600px; margin: 40px auto; background-color: #ffffff; border: 1px solid #E7E1D8; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 12px rgba(46, 37, 32, 0.03);">
        ...
        <div style="padding: 40px 32px; text-align: center;">
            @if($status === 'approved')
                <h1 style="font-family: Georgia, Times, serif; font-size: 24px; font-weight: normal; color: #A2582F; margin-top: 0; margin-bottom: 20px;">Product Approved</h1>
                <p style="margin-top: 0; margin-bottom: 24px; font-size: 16px; color: #5C524A; line-height: 1.6;">
                    Your product listing <strong>{{ $productName }}</strong> has been approved and is now active in the marketplace.
                </p>
            @elseif($status === 'rejected')
                ...
                @if($reason)
                    <div style="background-color: #F7F4F0; border: 1px solid #E7E1D8; border-radius: 8px; padding: 16px 20px; margin-bottom: 24px; text-align: left; font-size: 14px; color: #5C524A; line-height: 1.5;">
                        <strong style="color: #2E2520; display: block; margin-bottom: 4px;">Moderation Feedback:</strong>
                        {{ $reason }}
                    </div>
                @endif
            @else
                <h1 style="font-family: Georgia, Times, serif; font-size: 24px; font-weight: normal; color: #C2783F; margin-top: 0; margin-bottom: 20px;">Listing Flagged</h1>
                <p style="margin-top: 0; margin-bottom: 24px; font-size: 16px; color: #5C524A; line-height: 1.6;">
                    Your product listing <strong>{{ $productName }}</strong> has been flagged by administrators.
                </p>
                @if($reason)
                    <div style="background-color: #F7F4F0; border: 1px solid #E7E1D8; border-radius: 8px; padding: 16px 20px; margin-bottom: 24px; text-align: left; font-size: 14px; color: #5C524A; line-height: 1.5;">
                        <strong style="color: #2E2520; display: block; margin-bottom: 4px;">Moderation Feedback:</strong>
                ...
                @endif
            @endif
            <a href="{{ route('products.index') }}" style="display: inline-block; background-color: #A2582F; color: #ffffff; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 14px; font-weight: bold; text-decoration: none; padding: 12px 28px; border-radius: 6px; letter-spacing: 0.5px;">View My Products</a>
        </div>

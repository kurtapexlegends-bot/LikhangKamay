...
class GetTeamMessageThread
{
    ...
    public function execute(User $actor, TeamMessage $message): array
    {
        ...
        $replies = TeamMessage::query()
            ...
            ->map(function (TeamMessage $reply) use ($actor, $reactionFormatter) {
                return [
                    ...
                    'attachment_url' => $reply->attachment_url ?? ($reply->attachment_path ? \App\Services\StorageUrl::url($reply->attachment_path) : null),
        ...
        $parentData = [
            ...
            'attachment_url' => $message->attachment_url ?? ($message->attachment_path ? \App\Services\StorageUrl::url($message->attachment_path) : null),

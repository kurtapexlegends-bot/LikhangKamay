...
class B2BSupplyHubService
{
    ...
    /**
     * Retrieve listings data for the seller's published wholesale materials.
     */
    public function getMyListingsData(User $actor): array
    {
    ...
    /**
     * Update wholesale listing configuration and emit decoupled activity event.
     */
    public function updateProductWholesaleSettings(User $actor, Product $product, array $validated): string
    {
        $product->update([
        ...
        ProductWholesaleSettingsUpdated::dispatch($product, $actor, $validated, $isListed);
    ...
    /**
     * Retrieve cart overview data for material procurement.
     */
    public function getCartData(User $actor): array
    {
    ...
    /**
     * Retrieve checkout view data for procurement order.
     */
    public function getCheckoutData(User $actor, array $items): array
    {
        $myPublishedCount = Product::where('user_id', $actor->id)
            ->where('is_b2b_supply', DB::raw('true'))
            ->count();
    ...
    /**
     * Retrieve inbound B2B material sourcing orders.
     */
    public function getSourcingOrdersData(User $actor, Request $request): array
    {
    ...
    /**
     * Retrieve wholesale supply sales orders placed by peer artisans.
     */
    public function getWholesaleSalesData(User $actor, Request $request): array
    {
        $statusFilter = $request->input('status', 'all');
        $search = trim((string) $request->input('search', ''));
    ...
    /**
     * Retrieve printable purchase order / commercial invoice.
     */
    public function getInvoiceOrder(User $actor, string $id): Order
    {
        return Order::with([
